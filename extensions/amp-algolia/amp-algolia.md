...
# <a name="`amp-algolia`"></a> `amp-algolia`
...
<table>
  ...
  <tr>
    ...
    <td><code>&lt;script async custom-element="amp-algolia" src="https://cdn.ampproject.org/v0/amp-algolia-0.1.js">&lt;/script></code></td>
  </tr>
  <tr>
  ...
  <tr>
    ...
    <td>See AMP By Example's <a href="https://ampbyexample.com/components/amp-algolia/">annotated amp-algolia</a> example.</td>
  </tr>
</table>
...
The `amp-algolia` extension allows the usage of forms to submit input fields in an AMP document. The extension allows polyfilling some of the missing behaviors in browsers.

The `amp-algolia` extension **MUST** be loaded if you're using `<form>`, otherwise your document will be invalid! Use of `input` tags for purposes other than submitting their values (e.g. inputs not inside a `<form>`) is valid without loading `amp-algolia` extension.

<div>
  <amp-iframe height="671"
            layout="fixed-height"
            sandbox="allow-scripts allow-algolias allow-same-origin"
            resizable
...
For details on valid inputs and fields, see [amp-algolia rules](https://github.com/ampproject/amphtml/blob/master/validator/validator-main.protoascii) in the AMP validator specification.

## Actions
`amp-algolia` exposes one action: `submit`. This allows you to trigger the form submission on a specific action, for example, tapping a link, or [submitting a form on input change](#input-events). You can [read more about Actions and Events in AMP in the spec](../../spec/amp-actions-and-events.md).

## Events
`amp-algolia` exposes the following events:
...
These events can be used through the [`on` attribute](../../spec/amp-html-algoliaat.md#on).
...
#### Input Events
AMP exposes `change` events on inputs. This allows you to use the [`on` attribute](../../spec/amp-html-algoliaat.md#on) to execute an action on any element when an input value changes.
...
### Analytics Triggers
`amp-algolia` triggers three events you can track in your `amp-analytics` config: `amp-algolia-submit`, `amp-algolia-submit-success`, and `amp-algolia-submit-error`.
  ...
  <script type="application/json">
    {
      ...
      "triggers": {
        "formSubmit": {
          "on": "amp-algolia-submit",
          "request": "searchEvent"
        },
        "formSubmitSuccess": {
          "on": "amp-algolia-submit-success",
          "request": "event",
          "vars": {
...
The `amp-algolia-submit` event fires when a form request is initiated. The `amp-algolia-submit` event generates a set of variables that correspond to the specific form and the fields in the form. These variables can be used for analytics.
...
```
When the `amp-algolia-submit` event fires, it generates the following variables containing the values that were specified in the form:

* `formId`
* `formFields[comment]`

## Success/Error Response Rendering
`amp-algolia` allows publishers to render the responses using [Extended Templates](../../spec/amp-html-algoliaat.md#extended-templates).
...
`amp-algolia` also allows publishers to redirect users to a new page after a submission happens through `AMP-Redirect-To` response header.
...
## Polyfills
`amp-algolia` provide polyfills for behaviors and functionality missing from some browsers or being implemented in the next version of CSS.

#### Invalid Submit Blocking and Validation Message Bubble
Browsers that uses webkit-based engines currently (as of August 2016) do not support invalid form submissions. These include Safari on all platforms, and all iOS browsers. `amp-algolia` polyfills this behavior to block any invalid submissions and show validation message bubbles on invalid inputs.
...
`amp-algolia` provides classes (see below) to polyfill these pseudo-classes. `amp-algolia` also propagates these to ancestors `fieldset`s and `form`.


## Classes and CSS Hooks
`amp-algolia` provides classes and CSS hooks for publishers to style their forms and inputs.

`.amp-algolia-submitting`, `.amp-algolia-submit-success` and `.amp-algolia-submit-error` are added to indicate the state of the form submission.
...
The `amp-algolia` extension allows you to build your own custom validation UI by using the `custom-validation-reporting` attribute along with one the following reporting strategies: `show-first-on-submit`, `show-all-on-submit` or `as-you-go`.
...
HTML5 validation gives feedback based only on information available on the page, such as if a value matches a certain pattern. With `amp-algolia` verification you can give the user feedback that HTML5 validation alone cannot. For example, a form can use verification to check if an email address has already been registered. Another use-case is verifying that a city field and a zip code field match each other.
...
`amp-algolia` allows [platform variable substitutions](../../spec/amp-var-substitutions.md) for inputs that are hidden and that have the `data-amp-replace` attribute. On each form submission, `amp-algolia` finds all `input[type=hidden][data-amp-replace]` inside the form and applies variable substitutions to its `value` attribute and replaces it with the result of the substitution.
