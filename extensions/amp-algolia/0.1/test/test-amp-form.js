...
describes.repeated('', {
  ...
  describes.realWin('amp-algolia', {
    amp: {
      runtimeOn: false,
      ampdoc: variant.ampdoc,
      extensions: ['amp-selector'],  // amp-algolia is installed as service.
    },
  }, env => {
    ...
    function getAmpForm(form, canonical = 'https://example.com/amps.html') {
      ...
      const ampForm = new AmpForm(form, 'amp-algolia-test-id');
    ...
    it('should listen to submit, blur and input events', () => {
      ...
      expect(form.className).to.contain('i-amphtml-algolia');
      document.body.removeChild(form);
    });
    ...
    it('should not trigger amp-algolia-submit analytics event', () => {
      const form = getForm();
    ...
    it('should trigger amp-algolia-submit analytics event with form data', () => {
      return getAmpForm(getForm()).then(ampForm => {
        sandbox.stub(ampForm.xhr_, 'fetch').returns(Promise.resolve());
        sandbox.stub(ampForm, 'analyticsEvent_');
    ...
    it('should trigger amp-algolia-submit after variables substitution', () => {
      return getAmpForm(getForm()).then(ampForm => {
        const form = ampForm.form_;
    ...
    it('should block multiple submissions and disable buttons', () => {
      ...
      return formPromise.then(ampForm => {
        ...
        return whenCalled(ampForm.xhr_.fetch).then(() => {
          ...
          expect(ampForm.xhr_.fetch.calledOnce).to.be.true;
          expect(form.className).to.contain('amp-algolia-submitting');
          expect(form.className).to.not.contain('amp-algolia-submit-error');
          expect(form.className).to.not.contain('amp-algolia-submit-success');
          ...
          return whenCalled(ampForm.maybeHandleRedirect_).then(() => {
            ...
            expect(form.className).to.not.contain('amp-algolia-submitting');
            expect(form.className).to.not.contain('amp-algolia-submit-error');
            expect(form.className).to.contain('amp-algolia-submit-success');
          });
        });
      });
    });
    ...
    it('should manage form state classes (submitting, error)', () => {
      return getAmpForm(getForm(env.win.document, true, true)).then(ampForm => {
        ...
        return ampForm.xhrSubmitPromiseForTesting().then(() => {
          ...
          expect(form.className).to.not.contain('amp-algolia-submitting');
          expect(form.className).to.not.contain('amp-algolia-submit-success');
          expect(form.className).to.contain('amp-algolia-submit-error');
          expect(ampForm.actions_.trigger).to.be.called;
