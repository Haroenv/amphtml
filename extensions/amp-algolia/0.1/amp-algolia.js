...
import {FORM_VERIFY_PARAM, getFormVerifier} from './form-verifiers';
import {deepMerge} from '../../../src/utils/object';
import {AmpEvents} from '../../../src/amp-events';

/** @type {string} */
const TAG = 'amp-algolia';
...
export class AmpForm {
  ...
  constructor(element, id) {
    ...
    this.xhr_ = window.fetch; //Services.xhrFor(this.win_);
    ...
    this.form_.classList.add('i-amphtml-algolia');
  ...
  /**
   * Triggers 'amp-algolia-submit' event in 'amp-analytics' and
  ...
  triggerFormSubmitInAnalytics_() {
    ...
    this.analyticsEvent_('amp-algolia-submit', formDataForAnalytics);
  }
  ...
  handleXhrSubmit_(varSubsFields, trust) {
    ...
    const p = this.doVarSubs_(varSubsFields)
      .then(() => {
        ...
        this.actions_.trigger(this.form_, 'submit', /* event */ null, trust);
      ...
      .then(
        ...
        error => {
          return this.handleXhrSubmitFailure_(error);
        }
      );
   ...
   */
  doVerifyXhr_() {
    return true; //this.doXhr_(dev().assertString(this.xhrVerify_), this.method_, { [FORM_VERIFY_PARAM]: true, });
  }
  ...
  handleXhrSubmitSuccess_(response) {
    return response.json().then(
      json => {
        ...
        this.analyticsEvent_('amp-algolia-submit-success');
  ...
  handleXhrSubmitFailure_(error) {
    ...
    return promise.then(responseJson => {
      ...
      this.analyticsEvent_('amp-algolia-submit-error');
  ...
  setState_(newState) {
    ...
    this.form_.classList.remove(`amp-algolia-${previousState}`);
    this.form_.classList.add(`amp-algolia-${newState}`);
  ...
  renderTemplate_(data) {
    ...
    if (container) {
      ...
      if (this.templates_.hasTemplate(container)) {
        p = this.templates_
          .findAndRenderTemplate(container, data)
          .then(rendered => {
...
/**
 * Bootstraps the amp-algolia elements
 */
export class AmpFormService {
  ...
  constructor(ampdoc) {
    ...
    this.whenInitialized_ = this.installHandlers_(ampdoc);
  ...
  installSubmissionHandlers_(forms) {
    ...
    iterateCursor(forms, (form, index) => {
      ...
      if (!existingAmpForm) {
        new AmpForm(form, `amp-algolia-${index}`);
      }
    });
  }
